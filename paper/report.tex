...
\title{Fractional Step Solutions to Lid Driven Cavity Flow}% Force line breaks with \\

\author{Brooks Karlik}
 \altaffiliation{Mechanical Engineering Department, University of Nevada, Reno}%Lines break automatically or can be forced with \\
 \email{brooks@karlik.org}

\date{\today}% It is always \today, today,
             %  but any date may be explicitly specified
\begin{abstract}
	Fractional step methods can be applied to the Navier-Stokes equations to solve incompressible flow problems.
	In this work, we present an incompressible flow code written in Julia that solves the Navier-Stokes equations
	for a lid-driven cavity. We present velocity and vorticity distributions the verify the results of the solver
	with prior work. Code for the solver is provided on github \footnote{\url{https://github.com/VanillaBrooks/fractional-step}}.
%\begin{description}
%\item[Usage]
...
The fractional step method is a pressure correction numerical method that 
may be employed to solve the Navier-Stokes equations. Since common numerical 
methods will not necessarily guarantee that continuity is satisfied at the
next time step, an additional pressure field is calculated between step $n$ 
and $n+1$ to ensure that continuity is satisfied at the $n+1$ time step. This 
ensures that the numerical solutions of the flow are physical.

The Julia programming language is an open source scientific language rising in popularity 
based on the LLVM compiler backend. Julia provides easily accessible knobs to tune
the performance characteristics of code, including runtime measurements, benchmarking suites,
inlining functions, and eliding bounds checks on arrays. Julia also features a top-tier 
implementation of multiple dispatch to make programming interfaces ergonomic for scientists
and engineers. These characteristics were heavily leveraged throughout the solver to reduce 
code duplication and create generic plug-and-play couplings.

\section{\label{sec:level2} Experimental Setup}

The incompressible navier stokes equations \ref{eq_ns} and \ref{eq_ns_cont} are solved using the fractional step method.
The fractional step equations are provided in eqs \ref{eq_frac_1}, \ref{eq_frac_2}, \ref{eq_frac_3}.
...
The fractional step equations are derived using the 2nd order Adams Bashforth scheme
for the advection term, and the Crank Nicolson scheme for the viscous term.

The the domain is discretized into a $N\times N$ grid of $N=64$ cells.
No-slip boundary conditions placed on all faces of the domain except the 
top which is prescribed at $u = 1$ m/s. The flow is started from zero initial conditions
at a Reynolds number Re = 1000.

\begin{eqnarray}
	R u^F = S u^n + \Delta t (3 a^n - a^{n-1})
	\label{eq_frac_1}\\
	D R^{-1} G P^{n+1} 
	=
	\frac{D u^F}{\Delta t}
...
The linear systems exhibited
by the fractional step equations are solved using the conjugate gradient
method described in \cite{conj} to leverage their symmetric,
positive definite characteristics.

\section{\label{sec:level3} Results}

A velocity quiver plot is supplied in figure \ref{fig_velo} and compared to previous streamlines from \cite{ghia}.
A quiver plot is used in place of a streamline plot because I dont actually know how to calculate the streamline locations
for discrete data. The two results match the overall structure of the flow, including isolated vorticies in the bottom 
left and right corners of the flowfield. Currently, the authors suspect this is due to some underlying transposing
issues in the postprocessing routines.


The vorticity distribution of the flowfield is again compared to the results of \cite{ghia}
in figure \ref{fig_vort}. The overall distribution of the vorticity contours shows good agreeance with the 
previous numeric solution. Small differences in the plots can be explained by the size of the descritization since Ghia 
simulated $129^2$ points.

\begin{figure*}
	\includegraphics[]{./figs/velocity_compare.png}
	\caption{Our steady state velocity field (left) is compared with streamlines in \cite{ghia}}
	\label{fig_velo}
\end{figure*}


\begin{figure*}
	\includegraphics[]{./figs/vorticity_compare.png}
	\caption{Our steady state vorticity (left) is compared with \cite{ghia}}
	\label{fig_vort}
\end{figure*}

Tables \ref{table_u_vel} and \ref{table_v_vel} detail numerical solutions for two Reynolds numbers along 
the midspan x and y slices compared to \cite{ghia}. Results show only minimal discrepancies between the 
true solutions. Due to the solver running at a grid resolution of 80x80 points, values are roughly 
interpolated and extrapolated to the finer grid resolution of 129x129 points.
