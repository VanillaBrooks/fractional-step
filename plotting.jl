...
begin
	...
	velocityT = permutedims(velocity, (1,2,3,4))
	time = read(h5file["times"])
	#velocityT = reverse(velocityT, dims=4)
	# velocityT = reverse(velocityT, dims=)
	# velocity = copy(velocityT)
	#velocityT = reverse(velocityT, dims=)
end
...
begin
	local min_poss = minimum(velocityT[:, 1, :, :])
	local max_poss = maximum(velocityT[:, 1, :, :])
...
begin
	...
	local vx = reshape(vx, nx*ny)
	local vy = reshape(vy, nx*ny)

	x_grid = repeat(1:nx, 1, ny) |> vec
	y_grid = transpose(repeat(1:nx, 1,ny)) |> vec
...
begin
	local anim = @animate for write = 1:num_writes
		...
		local vx = reshape(vx, nx*ny)[slice]
		local vy = reshape(vy, nx*ny)[slice]
