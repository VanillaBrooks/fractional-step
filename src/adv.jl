module advection
	...
	function adv(dims::Dims, bcs::BoundaryConditions, iu::Matrix{Int64}, iv::Matrix{Int64}, ip::Matrix{Int64}, q::Vector{Float64})::Vector{Float64}
		nq = zeros(dims.nu)
		nx = dims.nx
		ny = dims.ny
	...
	reg_bar_left(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i-1,j]] + q[indexer[i,j]])
	reg_bar_right(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i+1,j]] + q[indexer[i,j]])

	reg_bar_top(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i,j+1]] + q[indexer[i,j]])
	reg_bar_bot(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i,j-1]] + q[indexer[i,j]])
	...
	x_dir_v_bar_top(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		# dont need j+1 here since values at y=j are already above the current i value
		(1/2) * (q[indexer[i+1,j]] + q[indexer[i,j]])

	x_dir_v_bar_bot(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
	...
	x_dir_v_bar_top_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (bcs.v_t[i] + bcs.v_t[i+1])

	# bottom boundary indexing for y
	x_dir_v_bar_bot_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (bcs.v_b[i] + bcs.v_b[i+1])

	
	# bot average with the ghost node is just the boundary condition
	x_dir_u_bar_bot_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		bcs.u_b[i]

	# top average with the ghost node is just the boundary condition
	x_dir_u_bar_top_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		bcs.u_t[i]
	...
	x_dir_u_bar_left_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i,j]] + bcs.u_l[i])

	x_dir_u_bar_right_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
	...
	y_dir_u_bar_left(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (q[indexer[i-1,j]] + q[indexer[i-1,j+1]])

	y_dir_u_bar_right(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
	...
	y_dir_v_bar_bot_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (bcs.v_b[i] + q[indexer[i,j]])

	y_dir_v_bar_top_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
	...
	y_dir_v_bar_left_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		bcs.v_l[j]

	y_dir_v_bar_right_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		bcs.v_r[j]

	y_dir_u_bar_left_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
		(1/2) * (bcs.u_l[j] + bcs.u_l[j+1])

	y_dir_u_bar_right_bc(q::Vector{Float64}, indexer::Matrix{Int64}, bcs::BoundaryConditions, i::Int, j::Int)::Float64 =
